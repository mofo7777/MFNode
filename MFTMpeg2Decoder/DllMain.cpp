...
ClassFactoryData g_ClassFactories[] = {{ &CLSID_MFTMpeg2Decoder, CMFTVideoMpeg2::CreateInstance }};

const DWORD g_numClassFactories = ARRAY_SIZE(g_ClassFactories);

const WCHAR SZ_DECODER_NAME[] = L"MFNode Mpeg2 Decoder";
...
STDAPI DllRegisterServer(){
	...
	if(SUCCEEDED(hr = RegisterObject(g_hModule, CLSID_MFTMpeg2Decoder, SZ_DECODER_NAME, TEXT("Both")))){

		// MFVideoFormat_MPG1 MFVideoFormat_MPEG2
		MFT_REGISTER_TYPE_INFO aDecoderInputTypes[] = {{ MFMediaType_Video, MFVideoFormat_MPEG2 }, { MFMediaType_Video, MFVideoFormat_MPG1 }};
		MFT_REGISTER_TYPE_INFO aDecoderOutputTypes[] = {{ MFMediaType_Video, NULL }};

		// Register the decoder for MFTEnum(Ex).
		hr = MFTRegister(CLSID_MFTMpeg2Decoder,                    // CLSID
...
STDAPI DllUnregisterServer(){
	...
	UnregisterObject(CLSID_MFTMpeg2Decoder);
	MFTUnregister(CLSID_MFTMpeg2Decoder);
