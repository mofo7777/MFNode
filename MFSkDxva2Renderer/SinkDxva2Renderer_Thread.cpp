...
DWORD WINAPI CSinkDxva2Renderer::StaticThreadProc(LPVOID lpParameter){

		CSinkDxva2Renderer* pRenderer = reinterpret_cast<CSinkDxva2Renderer*>(lpParameter);
...
HRESULT CSinkDxva2Renderer::StartRenderer(){
...
HRESULT CSinkDxva2Renderer::StopRenderer(){
...
DWORD CSinkDxva2Renderer::RendererThreadProc(){
				...
				if(dwResult == WAIT_TIMEOUT){
						...
						if(bPlaying){
						  ...
						  if(IsTimeToPresentSample(pSampleToRender, lWait)){
										...
										hr = m_cDxva2Manager.OnFrameMove(pSampleToRender);
						    m_cMFSafeQueue.PopSample();
								  ...
								  hr = m_pStreamDxva2Renderer->RequestSample();
						...
						hr = m_cDxva2Manager.OnFrameRender();
...
BOOL CSinkDxva2Renderer::IsTimeToPresentSample(IMFSample* pSample, LONG& lWait){
