...
HRESULT CMFByteStreamHandler::CreateInstance(IUnknown* pUnkOuter, REFIID iid, void** ppv){
		...
		CMFByteStreamHandler* pHandler = new (std::nothrow)CMFByteStreamHandler();
...
HRESULT CMFByteStreamHandler::QueryInterface(REFIID riid, void** ppv){
		
		TRACE_HANDLER((L"ByteStream::QI : riid = %s", GetIIDString(riid)));

		static const QITAB qit[] = {
				QITABENT(CMFByteStreamHandler, IMFByteStreamHandler),
				{0}
		};
...
ULONG CMFByteStreamHandler::AddRef(){
...
ULONG CMFByteStreamHandler::Release(){
...
HRESULT CMFByteStreamHandler::BeginCreateObject(IMFByteStream* pByteStream, LPCWSTR /*pwszURL*/, DWORD dwFlags, IPropertyStore* /*pProps*/,
		...
		CMFMpeg12Source* pSource = NULL;

		try{

				IF_FAILED_THROW(hr = CMFMpeg12Source::CreateInstance(&pSource));

				IF_FAILED_THROW(hr = MFCreateAsyncResult(NULL, pCallback, punkState, &pResult));

				IF_FAILED_THROW(hr = pSource->BeginOpen(pByteStream, this, NULL));
...
HRESULT CMFByteStreamHandler::EndCreateObject(IMFAsyncResult* pResult, MF_OBJECT_TYPE* pObjectType, IUnknown** ppObject){
...
HRESULT CMFByteStreamHandler::Invoke(IMFAsyncResult* pResult){
