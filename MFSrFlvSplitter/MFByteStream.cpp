...
HRESULT CMFByteStream::Initialize(LPCWSTR pwszFile, DWORD* pdwFlags){
		...
		// Need for Topoedit
		if((pos = m_wszFile.find(L"file", pos)) != std::wstring::npos){
...
HRESULT CMFByteStream::Read(BYTE* pData, const DWORD dwSize, DWORD* pdwRead){
		...
		if(ReadFile(m_hFile, pData, dwSize, pdwRead, 0)){
				hr = S_OK;
		}
		else{
...
HRESULT CMFByteStream::Seek(const LONGLONG lDistance){
		...
		DWORD dwPosition = SetFilePointer(m_hFile, (LONG)lDistance, NULL, FILE_CURRENT);
...
HRESULT CMFByteStream::Read(CMFReadParam* pReadPram){

		TRACE_BYTESTREAM((L"MFByteStream::Read"));

		HRESULT hr;

		if(ReadFile(m_hFile, pReadPram->GetDataPtr(), pReadPram->GetByteToRead(), pReadPram->GetpByteRead(), 0)){
				hr = S_OK;
		}
		else{
				hr = E_FAIL;
